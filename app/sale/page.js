  ...
  return (
    <div className='screen flex flex-col items-center'>
      <h1 className="font-extrabold flex items-center justify-between text-4xl w-full font-aboreto p-[10vh] border-b-2 h-fit">
        ...
        <Link href={'/'} className='text-slate-400 hover:text-white'><span className="material-symbols-outlined">
          close
        </span></Link>
      </h1>
      <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col p-4 m-4 bg-white/10 backdrop-blur-sm gap-4 justify-center h-[40%] my-auto w-1/2 rounded-xl'>
        ...
        <input type="submit" disabled={isSubmitting} value={isSubmitting ? 'Creating...' : 'Create Sale'} className='disabled:bg-sky-800/20 disabled:text-white mx-auto cursor-pointer w-[50%] rounded-3xl text-white/60 hover:text-white transition-all duration-300 font-sans font-bold hover:bg-sky-800 px-5 py-3 bg-sky-800/60' />
      </form>
    </div>
  )
}
